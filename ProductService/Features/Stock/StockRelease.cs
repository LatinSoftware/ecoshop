    ...
    public class StockRelease
    {
        ...
        public sealed class Handler(ApplicationContext context) : ICommandHandler<Command>
        {
            public async Task<Result> Handle(Command request, CancellationToken cancellationToken)
            {
                ...
                var result = product.ReleaseStock(request.Quantity);
                ...
                return Result.Ok().WithSuccess($"Product {product.Name} release correctly");
            }
        }
        ...
        public sealed class Endpoint : IEndpoint
        {
            ...
            {
                app.MapPost("products/{productId:guid}/release-stock", async (Guid productId, Command command, ISender sender) =>
                {
